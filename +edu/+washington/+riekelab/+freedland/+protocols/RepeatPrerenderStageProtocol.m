classdef (Abstract) RepeatPrerenderStageProtocol < edu.washington.riekelab.protocols.RiekeLabProtocol
    ...
    methods
        ...
        function controllerDidStartHardware(obj)
            ...
            if obj.waitingForHardwareToStart && obj.numEpochsPrepared == 1
                obj.waitingForHardwareToStart = false;
                obj.rig.getDevice('Stage').play(obj.createPresentation());
            elseif obj.waitingForHardwareToStart && obj.numEpochsPrepared > 1
                obj.waitingForHardwareToStart = false;
                obj.rig.getDevice('Stage').replay();
            end
        end
        
        function controllerDidStartHardwareDiskArray(obj)
            controllerDidStartHardwareDiskArray@edu.washington.riekelab.protocols.RiekeLabProtocol(obj);
            
            % specific
            if obj.waitingForHardwareToStart && obj.numEpochsPrepared <= 2
                obj.waitingForHardwareToStart = false;
                obj.rig.getDevice('Stage').play(obj.createPresentation());
            elseif obj.waitingForHardwareToStart && obj.numEpochsPrepared > 2
                obj.waitingForHardwareToStart = false;
                obj.rig.getDevice('Stage').replay();
            end
        end
