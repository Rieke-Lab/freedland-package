...
classdef RFFullFieldDiskFlash < edu.washington.riekelab.protocols.RiekeLabStageProtocol
    properties
        ...
        subunitIntegration = 'linear';
        centerSurroundIntegration = 'linear';
        ...
        function prepareRun(obj)
            ...
            for a = 1:length(obj.maskPresets)
                ...
                if ismember(presetNo,[1 2 3 4 11]) % no subunits
                    obj.integratedStat(a,1) = mean(obj.stat{a,1}); % center-surround integration
                elseif ismember(presetNo,[5 6 7 8 9 12]) % one subunit
                    temp = obj.stat{a,1};
                    centerSurround = [nanmean(temp(1:4)) nanmean(temp(5:8)) nanmean(temp(9:12))];
                    obj.integratedStat(a,1) = mean(centerSurround);   
                else % 2 subunits
                    temp = obj.stat{a,1};
                    centerSurround = [nanmean(temp(1:8)) nanmean(temp(9)) nanmean(temp(10))];
                    obj.integratedStat(a,1) = mean(centerSurround);   
                end
            end
