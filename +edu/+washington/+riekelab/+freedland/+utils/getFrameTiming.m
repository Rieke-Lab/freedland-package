function [frameTimes, frameRate] = getFrameTiming(frameMonitor,lcrFlag)
    ...
    if (lcrFlag == 0) %smooth out non-monotonicity (shows up on RigG)
       frameMonitor = edu.washington.riekelab.freedland.utils.lowPassFilter(frameMonitor, 360, 1/1e4); 
    end
    ...
    ups = edu.washington.riekelab.freedland.utils.getThresCross(frameMonitor,0.5,1);
    downs = edu.washington.riekelab.freedland.utils.getThresCross(frameMonitor,0.5,-1);
    ...
    if (lcrFlag == 1) %very fast, so frame times are clear directly from trace
       ...
       frameTimes = round(sort([timesOdd'; timesEven']));
        ...
        [~,flipsEven] = edu.washington.riekelab.freedland.utils.getPeaks(tempFM,1);
        [~,flipsOdd] = edu.washington.riekelab.freedland.utils.getPeaks(tempFM,-1);
